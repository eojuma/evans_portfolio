...
const request = async(signal:AbortSignal) => { const response=await fetch("/api/status",{signal}); if(!response.ok) throw new Error(); const data:unknown=await response.json(); if(!valid(data)) throw new Error(); return data };
